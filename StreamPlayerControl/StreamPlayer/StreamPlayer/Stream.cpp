...
void Stream::Open(string const& streamUrl)
{
	...
	if (avcodec_parameters_to_context(codecCtxPtr_, formatCtxPtr_->streams[videoStreamIndex_]->codecpar) < 0) {
		avcodec_close(codecCtxPtr_);
		avformat_close_input(&formatCtxPtr_);
		throw runtime_error("avcodec_parameters_to_context() failed");
	}
...
int avDecodePacket(AVCodecContext* s, AVFrame* frame, int* got_frame, AVPacket* pkt) {
	*got_frame = 0;
	int ret = 0;

	if (pkt) {
		ret = avcodec_send_packet(s, pkt);
	...
	ret = avcodec_receive_frame(s, frame);
...
unique_ptr<Frame> Stream::GetNextFrame()
{
	...
	while (!stopRequested_)
	{
		...
		if (avDecodePacket(codecCtxPtr_, avframePtr, &frameFinished, packetPtr) == 0) {
			if (frameFinished != 0) {
		...
		int bytesUsed = avDecodePacket(codecCtxPtr_, avframePtr, &frameFinished, packetPtr);
		if (bytesUsed < 0)
		{
			av_packet_unref(packetPtr);
			av_frame_free(&avframePtr);
